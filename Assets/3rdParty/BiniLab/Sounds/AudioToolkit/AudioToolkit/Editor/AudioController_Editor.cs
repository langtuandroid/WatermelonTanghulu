            ...
            if ( categoryFoldout = EditorGUILayout.Foldout( categoryFoldout, "Category Settings", foldoutStyle ) )
            {
                ...
                if ( curCat != null )
                {
                    ...
                    if ( itemFoldout = EditorGUILayout.Foldout( itemFoldout, "Audio Item Settings", foldoutStyle ) )
                    {
                        ...
                        // 선택한 AudioController에 등록된 오디오(clip) 리스트 추출
                        if (GUILayout.Button("리스트 뽑기", EditorStyles.miniButton))
                        {
                            StringBuilder sb = new StringBuilder();

                            foreach (AudioCategory cat in AC.AudioCategories)
                            {
                                sb.AppendLine($"---category : {cat.Name}");
                                foreach (AudioItem catAudioItem in cat.AudioItems)
                                {
                                    foreach (AudioSubItem audioSubItem in catAudioItem.subItems)
                                    {
                                        if (audioSubItem.Clip == null) continue;

                                        sb.AppendLine(audioSubItem.Clip.name);
                                    }
                                }
                            }
